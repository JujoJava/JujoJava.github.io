...
class ManejoBBDD
{
    ...
    /* CONEXIONES */
    public static function conectar_root(){
        try {
    ...
    public static function conectar_gestor(){
        try {
            self::$conexion = new PDO('mysql:host='.HOST.';dbname='.DBNAME,
                USERNAME_GESTOR, PASSWORD_GESTOR,
                array(PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES  \'UTF8\''));
        }
        catch (PDOException $e) {
            die("Ha habido un error al conectar: ".$e->getMessage());
    ...
    public static function conectar_cliente(){
        try {
            self::$conexion = new PDO('mysql:host='.HOST.';dbname='.DBNAME,
                USERNAME_CLIENTE, PASSWORD_CLIENTE,
                array(PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES  \'UTF8\''));
        }
        catch (PDOException $e) {
            die("Ha habido un error al conectar: ".$e->getMessage());
    ...
    public static function conectar_limitado(){
        try {
            self::$conexion = new PDO('mysql:host='.HOST.';dbname='.DBNAME,
                USERNAME_LIMITADO, PASSWORD_LIMITADO,
                array(PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES  \'UTF8\''));
        }
        catch (PDOException $e) {
            die("Ha habido un error al conectar: ".$e->getMessage());
